  ...
  if (monthGrid && yearValue) {
    ...
    async function loadRubricasFromSupabase() {
      try {
        ...
        const rubricas = await supabaseSelect("nb_rubrica", query);
        ...
        const allExpenses = await supabaseSelect("nb_despesa", expensesQuery);
        ...
        const allNotas = await supabaseSelect("nb_despesa_notas", notasQuery);
        ...
        const savedExpenses = localStorage.getItem("nbDynamicExpenses_" + selectedYear);
        if (savedExpenses) {
          try {
    ...
    function loadRubricasFromLocalStorage() {
      const savedRubricas = localStorage.getItem("nbDynamicRubricas_" + selectedYear);
      if (savedRubricas) {
        try {
    ...
    async function getNextRubricaId() {
      try {
        ...
        const result = await supabaseSelect("nb_rubrica", query);
    ...
    async function createRubricaInSupabase(name, tipo = "Despesa") {
      ...
      await supabaseInsert("nb_rubrica", data);
    ...
    async function updateRubricaSeqInSupabase(rubricaId, newSeq) {
      ...
      await supabaseUpdate("nb_rubrica", 
    ...
    async function deleteRubricaFromSupabase(rubricaId) {
      ...
      const notas = await supabaseSelect("nb_despesa_notas", notasQuery);
      for (const nota of notas) {
        await supabaseDelete("nb_despesa_notas", 
      ...
      const despesas = await supabaseSelect("nb_despesa", despesasQuery);
      for (const despesa of despesas) {
        await supabaseDelete("nb_despesa", 
          { ano: selectedYear, mes: selectedMonth + 1, rubrica_id: dbId, despesa_id: despesa.despesa_id }
        );
      }
      
      // Then delete the rubrica
      await supabaseDelete("nb_rubrica", 
        { ano: selectedYear, mes: selectedMonth + 1, rubrica_id: dbId }
      );
    }
    ...
    async function deleteRubricaFromAllMonths(rubricaId) {
      ...
      const rubricaMonths = await supabaseSelect("nb_rubrica", rubricaQuery);
      ...
      const allDespesas = await supabaseSelect("nb_despesa", despesasQuery);
    ...
    async function getNextDespesaId(rubricaDbId) {
      try {
        ...
        const result = await supabaseSelect("nb_despesa", query);
    ...
    async function createDespesaInSupabase(rubricaDbId, name, valor = 0, totalizador = true) {
      ...
      await supabaseInsert("nb_despesa", data);
    ...
    async function updateDespesaValueInSupabase(rubricaDbId, despesaDbId, valor) {
      await supabaseUpdate("nb_despesa", 
    ...
    async function deleteDespesaFromAllMonths(rubricaDbId, despesaDbId) {
      ...
      const despesaMonths = await supabaseSelect("nb_despesa", query);
    ...
    async function getNextContadorId(rubricaDbId, despesaDbId) {
      try {
        ...
        const result = await supabaseSelect("nb_despesa_notas", query);
    ...
    async function createDespesaNotaInSupabase(rubricaDbId, despesaDbId, valor, nota) {
      ...
      await supabaseInsert("nb_despesa_notas", data);
      return contadorId;
    }
    ...
    async function deleteDespesaNotasInSupabase(rubricaDbId, despesaDbId) {
      ...
      const notas = await supabaseSelect("nb_despesa_notas", query);
    ...
    function saveRubricas() {
      ...
      localStorage.setItem("nbDynamicExpenses_" + selectedYear, JSON.stringify(expensesData));
    }
